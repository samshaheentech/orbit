...
dir: ~/git/orbit
---
# Ops triage — label and archive, both accounts
...
## Steps, for each account (career first, then personal)
1. `search_emails` with `in:inbox newer_than:2d` (first run: `in:inbox newer_than:30d`), max 200. Build a JSON list `[{"id","from","subject","snippet","list_unsubscribe":<true if the message has a List-Unsubscribe header or an unsubscribe link in the snippet>,"labels":[...]}]` and write it to `state/operations/<account>-inbox.json` (create the folder).
