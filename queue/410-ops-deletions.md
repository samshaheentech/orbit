...
## Steps
0. Read `config/lanes/operations/accounts.json`. Only act on accounts where `active` is true. Skip inactive accounts silently, noted in STATUS, not as an error.
...
2. For each entry in `execute` whose account is active: in that account, `search_emails` with `from:<sender> -in:trash` (max 200), then `batch_modify_emails` with `addLabelIds: ["TRASH"]` and `removeLabelIds: ["INBOX"]` in batches of 50. Count what moved. Then `python3 lanes/operations/deletions.py executed --id <id> --count <n>`.
